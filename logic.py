...
def insert_player_at_end(name, surname, wins=0, loses=0, control_date=None):
    ...
    new_last_place = (
        ...
        else c.execute("SELECT Position FROM ranking;").fetchall()[-1][0] + 1
    )
    ...
    c.execute(
        "INSERT INTO ranking VALUES (?, ?, ?, ?, ?, ?);", new_player
    )  # Εκχώρηση παίκτη σε αυτή τη θέση
...
def insert_player_at_position(
    rank, name="", surname="", wins=0, loses=0, control_date=None
):
    """
    ...
    if is_position_empty(rank - 1) and rank != 1:
        ...
        if not is_position_empty(1):
            ...
            last_place = c.execute(
                "SELECT Position FROM ranking ORDER BY Position DESC LIMIT 1;"
            ).fetchone()[0]
            my_conn.close()
...
def change_position(old_position, new_position):
    ...
    query = cursor.execute(
        "SELECT * FROM ranking WHERE Position = ?;", (old_position,)
    )
...
def update_info(
    rank, name=None, surname=None, wins=None, loses=None, control_date=None
):
    """
    ...
    for field_name, field_value in fields:
        if field_value is not None:
            sql_query = (
                "UPDATE ranking SET {0} = ? WHERE Position = ?;".format(
                    field_name
                )
            )
            cursor.execute(sql_query, (field_value, rank))
    conn.commit()
    conn.close()
    return
...
def delete_player(index):
    ...
    c = my_conn.cursor()
    c.execute(
        "DELETE FROM ranking WHERE Position=?;", (index,)
    )  # Διαγραφή παίκτη
    c.execute(
        "UPDATE ranking SET Position = Position - 1 WHERE Position > ?;",
        (index,),
    )  # Ανανέωση λίστας
    my_conn.commit()
    my_conn.close()
...
def check_challenge(player_1, player_2):
    ...
    return {
        "success": True,
        "message": "Η πρόκληση είναι αποδεκτή.\nΝίκησε ο παίκτης στη θέση #{0} που έκανε την πρόκληση;".format(
            player_1
        ),
    }
...
def create_table():
    ...
    sql_query = (
        "CREATE TABLE IF NOT EXISTS ranking (Position INTEGER PRIMARY KEY, Name VARCHAR(128),"
...
def win(winner_index, loser_index, today_string=None):
    ...
    if loser_index < winner_index:
        ...
        c.execute(
            "INSERT INTO ranking(Position, Name, Surname, Wins, Loses, Control_Date) VALUES (?, ?, ?, ?, ?, ?);",
            entry_data,
        )
        my_conn.commit()
        my_conn.close()
...
def update_positions(small_num, big_num):
    ...
    for p in range(big_num, small_num - 1, -1):
        ...
        c.execute(
            "UPDATE ranking SET Position = ? WHERE Position=?", (p + 1, p)
        )
...
def rank_decay(index, today_string=None):
    ...
    c.execute(
        "UPDATE ranking SET Position = ? WHERE Position=?;", (index, index + 1)
    )
...
def check_ranking_for_decay(comparison_date=None):
    ...
    return_dict = {"immune": (None), "message": ""}
    ...
    if decay_list:
        decay_positions = ",".join(
            [
                str(single_decay_position)
                for single_decay_position in decay_list
            ]
        )
        if len(decay_list) == 1:
            decay_message = "".join(
    ...
    # Η λίστα αδρανών παικτών είναι κενή
        ...
        return_dict["message"] = (
            "Δεν υπάρχει κανένας παίκτης που να υπόκειται σε μείωση θέσης λόγω αδράνειας."
        )
    return return_dict
